...
static void setbits(unsigned char *map, unsigned startbit, unsigned nbits);
...
boot_alloc(uint32_t n)
{
	...
	// (4<<30)/4096/8 = 128k
	static unsigned char pagemap[128*1024];
	char *result;
	unsigned pagesalloc;
	unsigned startbit; 
	unsigned nbits;
...
#if 0
		// nextfree以下的部分均不能使用, 需要设置为1, 
		setbits(pagemap, 0, PADDR(nextfree)/PGSIZE);
		// >=npages的也不能用
		if (npages < sizeof(pagemap)<<3) { 
			// 此处存在bug, 物理内存目前只有4MB左右
			setbits(pagemap, npages, sizeof(pagemap)<<3);
		}
#endif
	}
	...
	pagesalloc = ROUNDUP(n, PGSIZE)>>PGSHIFT;
	startbit = PADDR(nextfree)>>PGSHIFT; 
	nbits = pagesalloc;
	if ((startbit+nbits) > npages) {
		panic("boot_alloc: no more memory can be allocated\n");
	}
#if 0	
	setbits(pagemap, startbit, nbits);
#endif 

	result = nextfree;
	nextfree += (pagesalloc<<PGSHIFT);
	return result;
}
...
static void setbits(unsigned char *map, unsigned startbit, unsigned nbits)
{
	...
	unsigned startbits = 8 - (startbit%8);
	unsigned endbyte = (startbit + nbits - 1)/8;
	unsigned endbits = (startbit + nbits)%8;
	// startbyte
	map[startbyte] |= (startbits<<1)-1;
	// 中间的完整部分
	for (unsigned i = startbyte+1; i < endbyte; ++i) {
		map[i] = 0xff;
	}
	// endbyte
	map[endbyte] |= ~(((8-endbits)<<1)-1);
}
...
mem_init(void)
{
	...
	//! kern_pgdir is aligned by PGSIZE, aka low 12bits are 0.
	...
	// Your code goes here:
	pgsize = PSE_PGSIZE;
	section_pages = ((4UL<<30) - KERNBASE)>>PSE_PGSHIFT;
	va = (void *)KERNBASE;
	pa = 0;
	for (unsigned i = 0; i < section_pages; ++i, va+=PSE_PGSIZE, pa+=PSE_PGSIZE) {
		if (((pa + PSE_PGSIZE)>>PGSHIFT) < npages) {
			struct Page *pp = pse_pa2page(pa);
			int ret;
			ret = pse_page_insert(kern_pgdir, pp, va, PTE_W);
			assert(!ret);
		} else {
			pde_t *pdep = pse_pgdir_walk(kern_pgdir, va);
			assert(pdep);
			*pdep |= (PTE_P | PTE_PS | PTE_W);
		}
	}
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		if ((i > 0 && i < npages_basemem) || i >= npages_struct_end) {
#ifdef DEBUG_PAGE_INIT			
			cprintf("<4>page_init: page %u in free\n", i);
#endif			
			pages[i].pp_ref = 0;
			pages[i].pp_link = page_free_list;
#if DEBUG_PAGE_INIT			
			cprintf("<4>page_init: page %u, pp_link %p\n", i, pages[i].pp_link);
#endif
			page_free_list = &pages[i];
			continue;
		} else {
#if DEBUG_PAGE_INIT			
			cprintf("<4>page_init: page %u in used\n", i);
#endif
			pages[i].pp_ref = 1;
		}
	}
...
{
	uint32_t pa;
	uint32_t numpages = PSE_PGSIZE>>PGSHIFT;
	// 从页表中删除对应条目，deref相关页
	if (pgdir[PDX(va)] & PTE_P) {
		pa = PTE_BASE(pgdir[PDX(va)])<<PSE_PGSHIFT;
		for (uint32_t i = 0; i < numpages; ++i) {
			page_decref(pa2page(pa + (i<<PGSHIFT)));
		}
	}
}

static inline void pse_page_incref(struct Page *pp, int step) 
{
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	pte_t *table = KADDR(PTE_BASE(dentry));
	pte_t *tentry = table + PTX(va);
	if (!(*tentry & PTE_P)) {
		return result;
	}
	result = pa2page(PTE_BASE(*tentry));
	if (pte_store) {
		*pte_store = tentry;
	}
	return result;
...
check_page_free_list(bool only_low_memory)
{
	...
	if (only_low_memory) {
		...
		for (pp = page_free_list; pp; pp = pp->pp_link) {
			// pdx_limit为4MB, 第0个entry对应[0, 4MB)
...
check_kern_pgdir(void)
{
	...
	// check phys mem
	pse_npages = (npages>>10);
	for (i = 0; i <  pse_npages*PSE_PGSIZE; i += PSE_PGSIZE)
...
check_va2pa(pde_t *pgdir, uintptr_t va)
{
	...
	if (!(*pgdir & PTE_P))
		return ~0;
	if (*pgdir & PTE_PS) {
		return PTE_BASE(*pgdir) | (va & 0x3fffff);
	}
	p = (pte_t*) KADDR(PTE_BASE(*pgdir));
	if (!(p[PTX(va)] & PTE_P))
		return ~0;
	return PTE_BASE(p[PTX(va)]);
}
...
check_page(void)
{
	...
	page_free(pp0);
	assert(page_insert(kern_pgdir, pp1, 0x0, PTE_W) == 0);
	assert(PTE_BASE(kern_pgdir[0]) == page2pa(pp0));
	...
	ptep = (pte_t *) KADDR(PTE_BASE(kern_pgdir[PDX(PGSIZE)]));
	...
	ptep1 = (pte_t *) KADDR(PTE_BASE(kern_pgdir[PDX(va)]));
